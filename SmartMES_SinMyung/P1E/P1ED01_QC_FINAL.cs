...
{
    public partial class P1ED01_QC_FINAL : SmartMES_SinMyung.FormBasic
    {
        ...
        public void ListSearch2(string rorderID)
        {
            lblMsg.Text = "";
            try
            {
                ...
                sP_QcFinal_QueryTableAdapter.Fill(dataSetP1E.SP_QcFinal_Query, rorderID, "%");

                dataGridView1.CurrentCell = null;
                dataGridView1.ClearSelection();

                dtpDate.Value = DateTime.Parse(dataGridView1[6, 0].Value.ToString());
        ...
        private void dataGridViewList_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            string sujuNo = dataGridViewList[0, index].Value.ToString();
            int qcCnt = Int32.Parse(dataGridViewList[7, index].Value.ToString());

            if (qcCnt > 0) ListSearch2(sujuNo);
            else ListInit();
        }
        ...
        private void pbSave_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(sQcMan) || sQcMan == "-")
            {
            ...
            for (int i = 0; i < dataGridView1.RowCount; i++)
            {
                ...
                sql = "INSERT INTO tb_qc_final(rorder_id, qc_item, qc_result, qc_decision, contents, qc_date, qc_man, enter_man) " +
                        "VALUES('" + sNo + "', '" + sItem + "', '" + sResult + "'," + sDecison + ",'" + sContents + "','" + sDate + "','" + sQcMan + "','" + G.UserID + "') " +
                        "ON DUPLICATE KEY UPDATE " +
                        "qc_result = '" + sResult + "', qc_decision = " + sDecison + ", contents = '" + sContents + "', qc_date = '" + sDate + "', qc_man = '" + sQcMan + "'";

                m.dbCUD(sql, ref msg);
                dataGridView1.Rows[i].Cells[8].Value = sNo;

                q++;
            }

            dataGridViewList[7, dataGridViewList.CurrentRow.Index].Value = q;
            lblMsg.Text = "저장되었습니다.";
        }
        private void pbPrint_Click(object sender, EventArgs e)
        {
            ...
            string rorderID = dataGridView1[8, 0].Value.ToString();
            sP_QcFinal_PrintTableAdapter.Fill(dataSetP1E.SP_QcFinal_Print, rorderID);

            string reportFileName = "SmartMES_SinMyung.Reports.P1ED01_QC_FINAL.rdlc";
