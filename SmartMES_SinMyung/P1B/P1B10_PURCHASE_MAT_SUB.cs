...
namespace SmartMES_SinMyung
{
    public partial class P1B10_PURCHASE_MAT_SUB : Form
    {
        ...
        private void lblRorderCust_Click(object sender, EventArgs e)
        {
            if (G.Authority == "D") return;

            CustFinder sub = new CustFinder();
            sub.FormSendEvent += new CustFinder.FormSendDataHandler(RorderCustEventMethod);
            sub._gubun = "103";
            sub.ShowDialog();
        }
        private void RorderCustEventMethod(object sender)
        {
            string sCust = sender.ToString();

            if (string.IsNullOrEmpty(sCust)) return;

            tbRorderCust.Tag = sCust.Substring(0, 7);
            tbRorderCust.Text = sCust.Substring(8, sCust.Length - 8);
        }
        ...
        private void Save()
        {
            ...
            string sRorderCust = string.Empty;
            string sProd = string.Empty;
            if (tbCust.Tag != null) sCust = tbCust.Tag.ToString();
            if (tbRorderCust.Tag != null) sRorderCust = tbRorderCust.Tag.ToString();
            ...
            if (lblTitle.Text.Substring(lblTitle.Text.Length - 4, 4) == "[추가]")
            {
                ...
                sql = "insert into tb_purchase_mat (purch_id, pos, putch_date, cust_id, request_date, prod_id, add_size, putch_qty, danga, rorder_cust, asap, enter_man1) " +
                    "values('" + sNo + "','" + G.Pos + "','" + sPutchDate + "','" + sCust + "','" + sRequestDate + "','" + sProd + "','" + sSize + "'," + sQty + "," + sDanga + ",'" + sRorderCust + "'," + sAsap + ",'" + G.UserID + "')";
                ...
                cbAsap.Checked = false;
                tbSize.Focus();
            }
            else
            {
                sql = "update tb_purchase_mat " +
                    "set putch_date = '" + sPutchDate + "', cust_id = '" + sCust + "', request_date = '" + sRequestDate + "', prod_id = '" + sProd + "', add_size = '" + sSize + "', putch_qty = " + sQty + ", danga = " + sDanga + ", rorder_cust = '" + sRorderCust + "', asap = " + sAsap +
                    " where purch_id = '" + sNo + "'";
